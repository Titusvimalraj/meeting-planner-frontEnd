...
export class SignUpComponent implements OnInit {
  ...
  getpasswordErrorMessage() {
    return this.passwordFormControl.hasError('required') ? 'You must enter a value' :
      this.passwordFormControl.hasError('minlength') ? 'minimum length is 5 characters' :
        this.passwordFormControl.hasError('pattern') ? 'password does not meet requirements should contain atleast one uppercase, one lowercase letter and a number' :
          '';
  }
  ...
  public signUp = () => {
    ...
    this.firstName = this.firstNameFormControl.value;
    this.lastName = this.lastNameFormControl.value;
    this.countryName = this.countryCode;
    this.mobileNumber = this.phoneCode;

    setTimeout(() => {

      if (!(this.email)) {
        this.toastr.info("please enter your email", "Info !");
        this.progressBar = false;
      }
      else if (this.appService.validateEmail(this.email)) {
        this.toastr.info("email does not meet the criteria", "Info !");
        this.progressBar = false;
      }
      else if (this.password !== this.confirmPassword.value) {
        this.toastr.warning("passwords do not match", "Error !");
        this.progressBar = false;
      }
      else if (this.appService.validatePassword(this.password)) {
        this.toastr.info("password does not meet the criteria", "Info !");
        this.progressBar = false;
      }
      else if (!(this.firstName)) {
        this.toastr.info("First Name cannot be empty", "Info !");
        this.progressBar = false;
      }
      else if (!(this.lastName)) {
        this.toastr.info("Last Name cannot be empty", "Info !");
        this.progressBar = false;
      }
      else if (!(this.countryName)) {
        this.toastr.info("Please select a country", "Info !");
        this.progressBar = false;
      }
      else if (!(this.mobileNumber)) {
        this.toastr.info("Mobile number is required", "Info !");
        this.progressBar = false;
      }
      else if (!(this.userType)) {
        this.toastr.info("Select a user Type", "Info !");
        this.progressBar = false;
      }
      else {
        let user = new User(this.firstName, this.lastName, this.email, this.password, this.countryName, this.mobileNumber, this.userType);
        this.appService.signUp(user).subscribe((apiResponse) => {
